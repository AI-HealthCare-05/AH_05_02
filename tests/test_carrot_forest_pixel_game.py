...
def test_pixel_game_is_installable_pwa() -> None:
    ...
    assert 'navigator.serviceWorker.register("/forest-sw.js", { scope: "/forest" })' in script
    assert 'window.addEventListener("beforeinstallprompt"' in script
    assert '"display": "standalone"' in manifest
    assert "forest-icon-192.png" in manifest
    assert "forest-icon-512.png" in manifest
