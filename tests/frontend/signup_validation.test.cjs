...
test('reports each missing password requirement without echoing the value', () => {
  for (const [value, expected] of [['12345678!', '영문자'], ['Abcdefgh!', '숫자'], ['Abcdef123', '특수문자'], ['Ab1!', '8자']]) {
...
test('empty password reports all four requirements; whitespace is not a special character', () => {
  assert.equal(context.signupPasswordIssues('').length, 4);
...
test('server-tested password format has no missing requirements', () => {
  ...
  assert.equal(context.signupPasswordIssues('health1!').length, 0);
});
