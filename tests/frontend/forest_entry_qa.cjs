...
(async () => {
  ...
  try {
    for (const width of [1440, 380]) {
      ...
      assert.equal(await page.getByRole('heading', { name: '당근의 숲', exact: true }).count(), 1);
      assert.equal(await page.locator('#dashboard-eyebrow').innerText(), '함께하기');
      assert.equal(await page.locator('#dashboard-lead').innerText(), '가족·친구와 챌린지 수행 상태를 함께 확인해요.');
      assert.ok(await page.locator('#dashboard-together-mascot').isVisible());
      assert.equal(await page.locator('.together-header, .together-context-label, .forest-week-pill').count(), 0);
      await page.locator('#dashboard-together-mascot').evaluate(n => n.decode());
      await page.locator('.dashboard-hero').screenshot({ path: `tmp/ui-ux-qa/together-heading-${width}.png` });
      await page.evaluate(() => showWorkspace('home'));
      assert.ok(await page.locator('#dashboard-together-mascot').isHidden());
      await page.evaluate(() => showWorkspace('together'));
