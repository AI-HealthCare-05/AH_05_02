...
from src.ml.modeling.knhanes_team1_six_feature_benchmark import derive_activity
...
def test_team1_activity_derivation_preserves_no_and_invalid_missing() -> None:
    frame = pd.DataFrame(
        {
            "record_key": ["a", "b", "c"],
            "BE3_75": [2, 1, 1],
    ...
    result = derive_activity(frame)
    assert result.loc[0, "leisure_aerobic_moderate_equivalent_min_week"] == 60.0
    assert result.loc[1, "leisure_aerobic_moderate_equivalent_min_week"] == 120.0
    assert np.isnan(result.loc[2, "leisure_aerobic_moderate_equivalent_min_week"])
    assert result.loc[0, "strength_days_week"] == 0.0
    assert result.loc[1, "strength_days_week"] == 5.0
    assert np.isnan(result.loc[2, "strength_days_week"])
