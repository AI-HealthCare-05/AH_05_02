...
CATALOG = (
    {"code": "sprout_hat", "name": "새싹 모자", "category": "hat", "price_carrots": 0, "asset_ref": "avatar/sprout-hat"},
    {"code": "round_glasses", "name": "동그란 안경", "category": "accessory", "price_carrots": 40, "asset_ref": "avatar/round-glasses"},
...
class GameService:
    ...
    async def inventory(self, user: User):
        ...
        return {"items": [{**self.item_payload(item_map[row.item_id]), "quantity": row.quantity, "acquired_at": row.acquired_at} for row in rows if row.item_id in item_map]}
    ...
    async def purchase(self, user: User, item_id: int):
        ...
        async with in_transaction():
            ...
            await self.repo.credit(user.id, -item.price_carrots, "inventory_purchase", str(item.id), f"purchase:{user.id}:{item.id}")
            await UserInventory.create(user_id=user.id, item_id=item.id, quantity=1)
        return {"item_id": item.id, "already_owned": False, "carrot_balance": (await self.repo.wallet(user.id)).carrot_balance}

    async def avatar(self, user: User):
        avatar = await self.repo.avatar(user.id)
        return {"equipped_item_ids": avatar.equipped_item_ids, "version": avatar.version, "updated_at": avatar.updated_at}
    ...
    async def equip(self, user: User, request: AvatarEquipRequest):
        ...
        if len(categories) != len(set(categories)):
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="같은 종류의 아이템은 하나만 장착할 수 있습니다.")
        avatar = await self.repo.avatar(user.id)
    ...
    def item_payload(item: InventoryItem):
        return {"item_id": item.id, "code": item.code, "name": item.name, "category": item.category, "price_carrots": item.price_carrots, "asset_ref": item.asset_ref}
