...
- 오늘의 퀘스트는 식단·운동과 함께 H02 물 마시기·수분 기록 챌린지를 고정으로 포함한다. 개인 음료 목표가 확인된 경우에만 200mL 물 아이콘 개수 체크로 표시한다. 컵별 탭 진행은 브라우저 새로고침 후 유지되며, 최종 인증은 서버의 단일 회차 기록과 기존 보상 고유키로 처리해 중복 보상을 막는다.
...
5. H02 고정 포함, 0mL/구간별 합산/양에 따른 추가 보상 없음, 확인된 개인 목표의 200mL 물 아이콘 체크, 중복 동시 완료 요청.
