...
- 선택 동의를 받은 사진에 한해 `OPENAI_VLM_FALLBACK_ENABLED=true`일 때 경계 구간(음식 40~50%, 채소 20~30%) 또는 로컬 모델 취약 결과만 OpenAI VLM으로 보완한다. VLM은 비율·영양·섭취 여부를 추정하지 않으며 관련성·채소 가시성·화질만 구조화해 반환한다. 명확한 통과·실패에는 호출하지 않고, VLM 장애·불일치 결과는 자동 승인하지 않는다.
