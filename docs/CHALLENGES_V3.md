...
| --- | --- | --- | --- |
| 1유형: 사진 + 채소 확인 | 대표 식사 사진 1장과 실제 실천 끼니 수 | 유효한 이미지, 자기보고 값이 목표 이상인지, 사진의 음식 면적과 음식 내부의 채소 면적이 기준을 충족하는지 | 실제 섭취, 모든 끼니의 실천, 섭취량, 비전분 채소의 정확한 종류, 영양성분·칼로리·치료 효과 |
...
1유형은 로컬 세그멘터가 계산한 전체 사진 대비 음식 면적이 50% 이상이고, 음식 픽셀 중 눈에 보이는 채소 면적이 30% 이상이며, 신뢰 불가 신호가 없을 때 확인 가능한 초안을 만든다. 사용자가 결과를 최종 확인한 뒤에만 승인하고 챌린지를 완료한다. 두 비율은 중량·열량·영양성분이나 실제 섭취량이 아닌 사진상의 픽셀 면적 추정치다. 임계값은 제품의 초기 운영 기준이며 별도의 정확도나 성공 확률을 뜻하지 않는다. 한식 분류 결과가 김치·찌개·조림 등 알려진 취약 요리에 해당하거나 유효하지 않은 수치가 나오면 완료하지 않고 확인 필요 상태로 처리한다.

기준 미달은 HTTP 200의 `review_status: "rejected"`, 신뢰하기 어려운 결과는 `review_status: "needs_review"`로 반환하며 둘 다 `challenge_completed: false`다. 사용자 응답에는 측정 비율을 포함하지 않고 통과 여부와 재촬영 안내만 제공한다. 수치·임계값·모델 버전·불확실 사유는 사진과 개인정보를 제외한 운영 로그에만 기록한다. 2유형의 `accepted`는 사진 제출 접수를 뜻하며 AI 내용 검증 성공이 아니다.
...
- 1유형은 서버에 배치한 한식 분류기와 TFLite 세그멘터로 처리하며 사진을 외부 OpenAI Vision 서비스에 전송하지 않는다. 기존 OpenAI Vision 구현 파일은 호환성 참고를 위해 남겨 두지만 챌린지 인증 경로에서는 사용하지 않는다. 얼굴·주소·이름 등 사진 안에 보이는 개인정보는 자동 삭제하지 않으므로 촬영 안내가 필요하다.
- `FOOD_VISION_PROVIDER=openai`로 잘못 설정해도 제공자 생성 단계에서 거부하며 외부 API를 호출하지 않는다.
- 공개 Google Drive 번들을 사용하는 배포는 `KFOOD_DRIVE_FILE_ID`, `KFOOD_BUNDLE_SHA256`, `KFOOD_BUNDLE_DIR`을 설정한다. API 컨테이너는 시작 시 ZIP을 한 번 내려받고 번들 및 개별 모델 SHA-256을 검증한 뒤 쓰기 가능한 로컬 볼륨에 원자적으로 설치한다. 검증이나 다운로드가 실패하면 API를 시작하지 않으며, 요청 처리 중에는 Drive에 접근하지 않는다. 링크 공개 범위만큼 모델 파일도 공개된다는 점과 Drive 트래픽 제한을 운영 위험으로 관리한다.
- 로컬 분류기와 TFLite 인터프리터는 API 프로세스 안에서 재사용한다. readiness는 단순 파일 존재가 아니라 개별 SHA-256과 모델 설정 JSON을 검증하며, 운영 중 모델 파일이나 설정을 바꾸려면 컨테이너를 재시작한다.
- `OPENAI_VLM_FALLBACK_ENABLED=true`이면 경계 구간(음식 40~50%, 채소 20~30%) 또는 로컬 모델 취약 결과만 OpenAI VLM으로 자동 보완한다. 제출 시 별도 선택 동의를 받지 않으므로 업로드 화면과 개인정보 처리방침에 외부 AI 전송 가능성을 명확히 고지해야 하며, 운영 활성화 전 법률·개인정보 검토를 완료한다. VLM은 비율·영양·섭취 여부를 추정하지 않으며 관련성·채소 가시성·화질만 구조화해 반환한다. 명확한 통과·실패에는 호출하지 않고, VLM 장애·불일치 결과는 자동 승인하지 않는다.
...
`photo_review_available`은 로컬 한식 분류 모델, 메타데이터, TFLite 세그멘터, 설정 파일과 체크섬이 모두 준비됐는지를 나타낸다. 모델이 실제 사진을 정확히 판정한다는 보증은 아니다. `medical_guidance_required_first`가 참이면 의료기관 안내 확인이 먼저 필요하다. `catalog_version`, `recommendation_type`, `personalized`, `preference_applied`, `notice`도 확인한다.
...
| 502 | 로컬 모델 로딩·추론 실패 또는 잘못된 결과 | 완료 처리하지 않음; 모델 파일·체크섬·런타임 확인 후 재시도 |
