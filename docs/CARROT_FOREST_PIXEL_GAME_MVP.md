...
- 하단 조작부는 사진 스케치에 맞춘 3×3 오락실 패드로 구성한다. 배치는 `J 점프–위–R 달리기 / 왼쪽–Q 상호작용–오른쪽 / E 탈것–아래–Z 공격`이며 가운데 `Q`는 노란색, 점프·공격은 빨간색, 달리기·탈것은 파란색이다. 채팅·댄스·앉기·펫 먹이는 패널에서 제외하고 기존 키보드 단축키와 대상 직접 클릭 방식은 유지한다.
