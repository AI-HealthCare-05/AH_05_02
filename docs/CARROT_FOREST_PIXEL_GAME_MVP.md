...
- 사용자 현지 시각에 따른 낮·황혼·밤 조명과 야간 등불·모닥불 주변광. 야간 음영은 지도 배경에만 적용해 캐릭터와 배치 오브젝트 본체가 가려지지 않는다.
...
- `F` 또는 오락실형 조작 패널의 `펫 먹이` 버튼으로 보유 당근 1개를 펫에게 줄 수 있고, 펫 머리 위에 하트 이펙트가 나타난다.
- 펫이 있는 상태에서 야생 쥐가 플레이어 반경 안으로 접근하면 펫이 자동 추적·공격하고 당근 보상을 가져온다. 직접 `Z` 공격도 그대로 사용할 수 있다.
- 하단 조작부는 방향 패드와 색상별 액션 버튼을 갖춘 오락실 컨트롤 덱으로 구성하며, `X`는 키 반복 없이 앉기/서기 상태를 토글한다.
