...
- `R` 달리기, `C` 채팅, `X` 앉기·일어서기 토글, `E` 탈것 승하차, `J` 점프
...
- 배치한 의자나 벤치를 클릭·탭하거나 가까이에서 `Q`를 누르면 해당 자리에 정렬해 앉는다. `X`는 메인 필드와 집 안에서 한 번 누르면 앉고 다시 누르면 일어나는 단일 토글이다. 메인 필드에서는 바닥 앉기 완성 자세, 집 안에서는 실내 휴식 자세를 사용하며 당근밭에서는 비활성화한다.
