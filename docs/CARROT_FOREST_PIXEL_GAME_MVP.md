...
- Last tick 무료 고양이는 회색·흰색·치즈색 3종을 제공하고, 무료 겨울 4종·밸런타인 11종 장비를 펫 카테고리 안의 별도 장비 그룹에서 선택한다. 정지 후 10초 앉기, 30초 앞발을 넣고 쉬기, 60초 앉아 자기, 90초 머리를 내리고 자기, 120초 몸을 뻗어 자기로 원본 휴식 프레임이 단계적으로 바뀐다.
...
Phaser 월드에서 분수·반딧불 랜턴·바람개비는 원화의 첫 프레임을 기준으로 사용한다. 원화의 다른 프레임은 돌·잔디·기둥까지 다시 그려져 있으므로 전체 프레임 반복 재생을 하지 않는다. 분수는 물방울 하이라이트만 세로 ±0.35px, 반딧불 랜턴은 별도 빛 점만 가로 ±0.65px·세로 ±0.4px 움직이며, 배치 컨테이너와 받침은 이동·회전·확대하지 않는다. 랜턴을 끄면 추가 빛 점도 즉시 지운다. 바람개비는 정규화된 첫 프레임에서 날개만 별도 텍스처로 분리해 고정된 중심을 기준으로 36초에 한 바퀴 천천히 회전한다. 기둥·꽃·돌은 고정하며 날개 뒤에 가려진 기둥은 원래 기둥 픽셀로 연결한다.

오리는 별도 투명 오리 자산만 세로 ±0.45px로 천천히 움직인다. 수평 이동·회전·크기 변화는 없다. 젖소는 본체만 지속 동작 1px 미만, 터치 반응 최대 1.8px로 움직이고 잔디 받침은 고정한다. 본체의 `setDisplaySize`에서 계산된 원래 축척을 보존하며 `setScale(1)`로 원본 크기로 커지는 현상을 막는다. 모든 수치는 768×512 월드 좌표 기준이며 `prefers-reduced-motion: reduce`에서는 오리·젖소 동작, 바람개비 회전과 물·빛 점의 좌표 변화를 정지한다. 클릭·Q·ON/OFF와 배치 상태 저장은 유지한다.

회귀 검증: `node --test tests/forest-object-motion.test.cjs tests/forest-fire.test.cjs tests/forest-pointer-controls.test.cjs tests/forest-running.test.cjs`.
