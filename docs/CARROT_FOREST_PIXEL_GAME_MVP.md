...
- 하단을 `조작 버튼 | 옷장 | 창고` 3분할로 구성해 이동과 꾸미기를 한 화면에서 사용
- 옷장·창고는 한 줄 높이의 가로 캐러셀로 표시하며 마우스 휠로 좌우 탐색
- 상단 `BGM` 버튼과 배경음악·효과음별 음량 슬라이더 및 개별 음소거, `날씨·시간` 표시 효과 제공
...
- 아바타 액세서리 장착·해제
- 숲 오브젝트를 선택하면 32px 격자로 배치 가능·불가능 영역을 표시한다. 격자 위치 선택 후 반투명 미리보기를 90도씩 회전하고, `V` 키 또는 `배치 확정` 버튼을 눌러야 위치·방향을 저장한다. 확정 전에는 취소할 수 있으며 배치 후 창고 회수도 지원한다.
...
- 정적 창고 오브젝트도 Phaser 월드 액터로 렌더링해 선택·필드 클릭·배치·회수가 화면과 상태에서 일치한다. 비균등 원본 시트는 연결 알파 컴포넌트 기준으로 분리한 256px 안전 셀 아틀라스로 교체해 이웃 오브젝트 픽셀 노출을 막는다.
...
- 배치한 의자나 벤치를 클릭·탭하거나 가까이에서 `Q`를 누르면 해당 자리에 정렬해 앉는다. `X`는 화면 버튼 없이도 사용할 수 있는 앉기·일어서기 보조 단축키로 유지한다.
...
- 월드의 펫을 클릭·탭하면 보유 당근 1개를 줄 수 있고, 펫 머리 위에 하트 이펙트가 나타난다. `F`는 화면 버튼 없이도 사용할 수 있는 보조 단축키로 유지한다.
- 펫이 있는 상태에서 야생 쥐가 플레이어 반경 안으로 접근하면 펫이 자동 추적·공격하고 당근 보상을 가져온다. 직접 `Z` 공격도 그대로 사용할 수 있다.
- 하단 조작부는 사진 스케치에 맞춘 3×3 오락실 패드로 구성한다. 배치는 `점프–위–달리기 / 왼쪽–상호작용–오른쪽 / 탈것–아래–공격`이며 버튼에는 단축키가 아닌 행동명을 표시한다. 가운데 상호작용은 노란색, 점프·공격은 빨간색, 달리기·탈것은 파란색이다. 어두운 안내 문구 영역은 제거하고 흰색 작업 도구 베이스 오른쪽에 실제 아바타 편집기와 인벤토리를 여는 `옷장·창고` 바로가기를 둔다. 채팅·댄스·앉기·펫 먹이는 패널에서 제외하고 기존 키보드 단축키와 대상 직접 클릭 방식은 유지한다.
- 로컬 데모에서만 `?resetToday=1`로 오늘 과업·공동 진행·보물상자 수령 상태를 초기화할 수 있으며 아바타·인벤토리·배치 오브젝트는 유지한다.
- LPC 기본 남성형은 짙은 청색 헤어와 검은 정장, 기본 여성형은 푸른 장발·검은 모자·주황 상의를 사용한다. 얼굴·표정 레이어를 기본 활성화하고 피부 선택은 머리를 포함한 몸과 코 레이어에 함께 적용한다.
...
- `src/frontend/assets/carrot-forest-storage-atlas-v3.png`: 각 오브젝트를 독립 256px 안전 셀로 분리한 실제 실행용 5×4 아틀라스
...
- `src/frontend/assets/home-record-elfwood-nexon.mp3`, `home-record-untitled-hoon.mp3`, `home-record-bright-time-sam.mp3`, `home-record-warm-afternoon-hyuk.mp3`: 사용자가 제공한 LP 선택곡 `Elfwood`, `무제`, `밝은 시간`, `따뜻한 오후`. 곡별 표시 크레딧을 유지하며 외부 배포 전 사용 권한을 별도로 확인한다.
...
- `src/frontend/assets/sfx/*.wav`: 걷기·달리기·문 열기·앉기·탑승·수확·물주기·낚시·무기별 공격·쥐 처치·펫 먹이·댄스·오브젝트 배치·오브젝트 켜기/끄기에 사용하는 오리지널 효과음 19종이다. 외부 음원을 복제하지 않고 `scripts/generate_forest_sfx.py`의 발진기·노이즈 합성으로 생성하며, 배경음악과 분리된 `ForestSfx` 재생기로 짧게 겹쳐 재생한다.
...
월드에 배치한 행운의 젖소·모닥불·숲 등불·반딧불 랜턴·전구 텐트는 개별 `active` 상태를 저장한다. 새로 배치하면 정지·소등 상태로 시작하며, 아바타가 가까이 간 뒤 `Q`를 누르거나 오브젝트를 클릭하면 즉시 켜지고 다시 조작하면 꺼진다. 젖소는 통통 움직이고, 모닥불은 빠르게 깜빡이며, 등불류는 느리게 밝아지는 애니메이션을 사용한다. 서버 연동 시에도 `placed[].active`를 그대로 보존해야 한다. 집 내부의 고정 LP 재생기는 클릭 또는 근거리 `Q`로 선택 창을 열어 5곡 중 하나를 재생하거나 끌 수 있으며, 선택곡과 재생 상태를 함께 저장한다.
