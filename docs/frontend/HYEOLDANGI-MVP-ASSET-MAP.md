...
2년·4년·6년 마커가 이동할 때 아래 얼굴 배지를 교체한다. 범주명은 이미지에 합성하지 않고 UI 텍스트로 함께 표시한다.

| 범주 | 자산 | 표정·색상 |
| --- | --- | --- |
| 낮음 | `hyeoldangi-face-low.png` | 초록 배지, 편안한 눈과 밝은 웃음 |
| 주의 | `hyeoldangi-face-caution.png` | 노란 배지, 살짝 걱정하며 확인을 권하는 표정 |
| 높음 | `hyeoldangi-face-high.png` | 절제된 붉은 배지, 화나거나 겁먹지 않은 차분한 확인 표정 |
...
- 미래 전망 얼굴 마커에는 `낮음`, `주의`, `높음` 텍스트를 인접 배치해 색각 이상 사용자도 범주를 확인할 수 있게 한다.
