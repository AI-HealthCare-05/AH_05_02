...
- `DEMO_MODE=true`의 기본값은 API·DB·프론트 연결만 검증하고 개인 판정값을 생성하지 않는다. 승인 Artifact가 공급된 제한된 데모 배포에서 `DEMO_ARTIFACT_INFERENCE_ENABLED=true`를 추가한 경우에만 FastAPI가 오늘이·내일이를 프로세스 시작 시 한 번 검증·적재하고 실제 선별 결과를 생성한다.
