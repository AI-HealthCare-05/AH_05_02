...
- 사용자는 표정·눈·눈썹·코·입을 따로 조합하지 않고, 위치 검수가 끝난 완성형 얼굴 프리셋을 선택한다. 내부에서는 각 프리셋의 값을 하나의 얼굴 좌표 계약으로 합성한다.
...
- 체형 카드는 실제 남성·여성·근육형·슬림형·아이 바디를, 피부 카드는 선택 색이 머리·귀·손·몸에 적용된 민머리 바디를 렌더링한다. 이모티콘과 단색 원은 선택 미리보기로 사용하지 않는다.
...
턱선은 목 시작점보다 한 픽셀 앞에 배치해 머리가 몸과 분리되어 뜨지 않고 몸을 살짝 덮도록 한다. 외부 캐릭터 세트로 교체할 때도 체형·피부·얼굴·헤어가 같은 프레임 규격과 방향별 앵커를 제공하는지 먼저 확인한다. 초상화 전용 또는 한 방향 전용 세트는 월드 이동 아바타 대체재로 사용하지 않는다.
