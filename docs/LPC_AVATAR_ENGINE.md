...
- 표정 16종과 눈·눈썹·코·입·주름 선택값을 하나의 얼굴 좌표 계약으로 합성한다.
- 기본·걷기·달리기·앉기·점프·감정표현·수확·낚시·문 열기·공격 동작
- 공격 효과에 맞춰 검은 휘두르기, 화살은 사격, 마법은 주문 시전, 잎 칼날은 찌르기 동작을 사용
- 정면·좌·우 방향에 각각 눈·눈썹·코·입·볼 앵커를 두고, 후면에서는 얼굴을 그리지 않는다.
- 헤어와 안경은 얼굴 뒤가 아니라 얼굴 합성 이후에 그려져 앞머리·안경 위치가 일관된다.
...
LPC 합성 캔버스의 실제 발바닥 좌표를 Phaser 컨테이너 원점에 맞춰 캐릭터 발과 그림자가 붙도록 한다. 남성·여성·근육형·슬림형·아이 바디 원본을 선택하고 몸 전체에 피부 필터를 적용한다. 얼굴의 윤곽·코·볼·주름도 동일한 피부 키의 명암 팔레트를 사용해 얼굴·귀·손이 서로 다른 색으로 보이지 않게 한다. 얼굴 세부 요소는 바디와 같은 `64×64` 좌표계를 사용하며 헤어·안경·모자보다 먼저 그린다.
