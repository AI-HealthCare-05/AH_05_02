(() => {
  ...
  class ApiForestAdapter {
    constructor(token, groupId, userId = null) {
      this.mode = "api";
      this.token = token;
    ...
    async load() {
      ...
      const questIds = challengeRows.map((item, index) => {
        ...
        const resolved = usedQuestIds.has(preferred)
          ? ["walk", "meal", "check"].find(id => !usedQuestIds.has(id)) || ["walk", "meal", "check"][index]
          : preferred;
      ...
      if (!next.members.length) next.members = [{ id: "me", name: next.avatar.name, completed: 0, target: 3, me: true }];
