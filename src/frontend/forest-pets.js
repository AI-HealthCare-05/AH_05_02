/* Last tick's official FREE kitten sheets + matching FREE Valentine bow.
...
((root) => {
  ...
  const assets = Object.freeze(["white", "gray", "ginger", "red-bow"].map(color => Object.freeze({
    key: `forest-kitten-${color}`,
    url: `/static/assets/animals/licensed-kittens/${color}.png?v=20260908-1`,
  ...
  const IDLE_CYCLE_MS = 12000;
  const actions = Object.freeze({
    idle: Object.freeze({ label: "앉아서 기다리기", description: "긴 앉은 휴식 사이에 짧은 울음·하품·세수를 하는 원본 프레임", sourceRows: Object.freeze([0, 28, 32, 36]) }),
  ...
  function pose(id, options = {}) {
    ...
    const result = { key: pet.key, frame, flipX: false, originX: ORIGIN.x, originY: ORIGIN.y,
      scale: pet.scale, action: done ? "sit" : action, clip, done, supportsSit: true, durationMs: actionDurations[action] };
    if (pet.overlayKey) result.overlay = { key: pet.overlayKey, frame, flipX: false,
