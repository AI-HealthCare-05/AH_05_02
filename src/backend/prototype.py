...
class PrototypeStore:
    ...
    def __init__(self) -> None:
        ...
        self._sequences = {"user": 100, "consent": 10, "eligibility": 30, "checkup": 500, "prediction": 900, "cycle": 40, "user_challenge": 700}
...
def signup(payload: SignupRequest) -> dict[str, object]:
    ...
    return envelope({**store.users[user_id], "access_token": f"prototype-user-{user_id}", "storage_notice": "프로토타입 메모리 세션이며 서버 재시작 시 삭제됩니다."})
...
def check_eligibility(payload: EligibilityRequest, authorization: str | None = Header(default=None)) -> dict[str, object]:
    ...
    result = {
        ...
        "guidance": "이 서비스는 진단 전 위험 선별용입니다. 증상이나 기존 진단이 있다면 의료기관과 상담해 주세요." if not eligible else "건강정보 입력을 계속할 수 있습니다.",
    }
...
def create_checkup(payload: HealthCheckupRequest, authorization: str | None = Header(default=None)) -> dict[str, object]:
    user_id = current_user(authorization)
    ...
    item = {"checkup_id": checkup_id, "user_id": user_id, "bmi": bmi, "age": eligibility["age"], **payload.model_dump(mode="json"), "created_at": now_iso()}
...
class MockDiabetesInferenceAdapter:
    ...
    def predict(cls, checkup: dict) -> tuple[dict, list[dict]]:
        contributions = [
            ...
            ("가족력", 0.45 if checkup["has_family_history_diabetes"] else 0.0, "가족력이 있으면 정기 검진이 더 중요합니다."),
            ("흡연", 0.35 if checkup["smoking_status"] == "current" else 0.0, "금연은 여러 만성질환 위험을 낮추는 데 도움이 됩니다."),
            ("신체활동", 0.4 if checkup["physical_activity_level"] == "none" else 0.22 if checkup["physical_activity_level"] == "insufficient" else -0.25, "가능한 범위에서 규칙적인 활동을 이어가세요."),
        ]
        ...
        risk_factors = [
            {"factor_name": name, "direction": "protective" if value < 0 else "risk", "importance": round(abs(value), 2), "description": description}
...
def create_prediction_job(payload: PredictionJobRequest, authorization: str | None = Header(default=None)) -> dict[str, object]:
    user_id = current_user(authorization)
    ...
    if payload.disease_type != "diabetes":
        raise HTTPException(status_code=409, detail="고혈압 모델은 확장 예정이며 현재 프로토타입에서는 사용할 수 없습니다.")
    prediction_id = store.next_id("prediction")
    prediction, factors = MockDiabetesInferenceAdapter.predict(checkup)
    item = {"prediction_id": prediction_id, "user_id": user_id, "checkup_id": payload.checkup_id, **prediction, "risk_factors": factors, "created_at": now_iso()}
...
CHALLENGES = [
    {"challenge_id": 1, "title": "식후 10분 천천히 걷기", "category": "activity", "daily_goal": "10분", "reason": "식후 가벼운 활동 습관을 만들어 봅니다.", "safety": "통증·어지럼이 있으면 멈추고 의료진과 상담하세요."},
    {"challenge_id": 2, "title": "하루 물 6잔 확인하기", "category": "hydration", "daily_goal": "6잔", "reason": "물을 마신 횟수를 기록하는 쉬운 습관입니다.", "safety": "수분 제한을 안내받았다면 의료진 지침을 우선하세요."},
    {"challenge_id": 3, "title": "채소 반찬 한 접시 더하기", "category": "diet", "daily_goal": "1회", "reason": "균형 잡힌 식사를 실천하기 위한 기록입니다.", "safety": "개별 식이 제한이 있다면 의료진 지침을 우선하세요."},
]


@router.get("/challenge-recommendations")
def challenge_recommendations(prediction_id: int = Query(), authorization: str | None = Header(default=None)) -> dict[str, object]:
    user_id = current_user(authorization)
...
def create_challenge_cycle(payload: ChallengeCycleRequest, authorization: str | None = Header(default=None)) -> dict[str, object]:
    user_id = current_user(authorization)
    ...
    item = {"cycle_id": cycle_id, "user_id": user_id, "prediction_id": payload.prediction_id, "week_number": 1, "duration_days": 28, "status": "active", "user_challenges": user_challenges, "created_at": now_iso()}
...
def upsert_challenge_log(user_challenge_id: int, log_date: date, payload: ChallengeLogRequest, authorization: str | None = Header(default=None)) -> dict[str, object]:
    user_id = current_user(authorization)
    ...
    log = {"user_challenge_id": user_challenge_id, "log_date": log_date.isoformat(), **payload.model_dump(), "updated_at": now_iso()}
...
def dashboard_summary(authorization: str | None = Header(default=None)) -> dict[str, object]:
    user_id = current_user(authorization)
    predictions = sorted((p for p in store.predictions.values() if p["user_id"] == user_id), key=lambda p: p["created_at"])
    ...
    return envelope({
        "latest_prediction": None if not latest else {"prediction_id": latest["prediction_id"], "predicted_stage": latest["predicted_stage"], "risk_score": latest["risk_score"], "is_mock": True},
        "risk_change_from_first": risk_change,
        "risk_change_calculation": "조회 시 최초·최신 예측값으로 계산",
        "active_cycle": None if not cycle else {"cycle_id": cycle["cycle_id"], "week_number": cycle["week_number"], "duration_days": cycle["duration_days"], "selected_count": len(cycle["user_challenges"])},
...
def active_models() -> dict[str, object]:
    return envelope({"items": [{"disease_type": "diabetes", "model_version": MockDiabetesInferenceAdapter.model_version, "status": "mock", "classes": ["normal", "prediabetes", "screening_advised"]}, {"disease_type": "hypertension", "status": "planned"}]})
